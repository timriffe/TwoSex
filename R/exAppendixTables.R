...
rMeanhmUS   <- local(get(load("Data/results/exMeanr/rUShm.Rdata")))[,1]
rMeangmUS   <- local(get(load("Data/results/exMeanr/rUSgm.Rdata")))[,1]
rMeanlmUS   <- local(get(load("Data/results/exMeanr/rUSlm.Rdata")))[,1]
rIPFhmUS    <- local(get(load("Data/results/exIPFr/rUSipfhm.Rdata")))[,1]
rSigmaUS    <- local(get(load("Data/results/exGoodmanr/rSigmaUS.Rdata")))
rSigma0US   <- rSigmaUS[,"r.0.r"]
rSigma.5US  <- rSigmaUS[,"r.5.r"]
rSigma1US   <- rSigmaUS[,"r.1.r"]
rCRhmUS     <- local(get(load("Data/results/exCPr/rUScp.Rdata")))[,1]
...
rMeanhmES   <- local(get(load("Data/results/exMeanr/rEShm.Rdata")))[,1]
rMeangmES   <- local(get(load("Data/results/exMeanr/rESgm.Rdata")))[,1]
rMeanlmES   <- local(get(load("Data/results/exMeanr/rESlm.Rdata")))[,1]
rIPFhmES    <- local(get(load("Data/results/exIPFr/rESipfhm.Rdata")))[,1]
rSigmaES    <- local(get(load("Data/results/exGoodmanr/rSigmaES.Rdata")))
rSigma0ES   <- rSigmaES[,"r.0.r"]
rSigma.5ES  <- rSigmaES[,"r.5.r"]
rSigma1ES   <- rSigmaES[,"r.1.r"]
rCRhmES     <- local(get(load("Data/results/exCPr/rEScp.Rdata")))[,1]
...
print(xtable(ES, digits = c(0,rep(4,10)), align = rep("c",11)),
        ...
        file = "latex/xtables/ESexALL.tex",floating=FALSE)
print(xtable(US, digits = c(0,rep(4,10)), align = rep("c",11)),
        sanitize.colnames.function = identity, 
        file = "latex/xtables/USexALL.tex",floating=FALSE)


# How's about SRB too?

SMeanhmUS   <- local(get(load("Data/results/exMeanr/rUShm.Rdata")))[,2]
SMeangmUS   <- local(get(load("Data/results/exMeanr/rUSgm.Rdata")))[,2]
SMeanlmUS   <- local(get(load("Data/results/exMeanr/rUSlm.Rdata")))[,2]
SIPFhmUS    <- local(get(load("Data/results/exIPFr/rUSipfhm.Rdata")))[,2]
rSigmaUS    <- local(get(load("Data/results/exGoodmanr/rSigmaUS.Rdata")))
SSigma0US   <- rSigmaUS[,"S.0.SRB"]
SSigma.5US  <- rSigmaUS[,"S.5.SRB"]
SSigma1US   <- rSigmaUS[,"S.1.SRB"]
SCRhmUS     <- local(get(load("Data/results/exCPr/rUScp.Rdata")))[,2]
# Spain
SMeanhmES   <- local(get(load("Data/results/exMeanr/rEShm.Rdata")))[,2]
SMeangmES   <- local(get(load("Data/results/exMeanr/rESgm.Rdata")))[,2]
SMeanlmES   <- local(get(load("Data/results/exMeanr/rESlm.Rdata")))[,2]
SIPFhmES    <- local(get(load("Data/results/exIPFr/rESipfhm.Rdata")))[,2]
rSigmaES    <- local(get(load("Data/results/exGoodmanr/rSigmaES.Rdata")))
SSigma0ES   <- rSigmaES[,"S.0.SRB"]
SSigma.5ES  <- rSigmaES[,"S.5.SRB"]
SSigma1ES   <- rSigmaES[,"S.1.SRB"]
SCRhmES     <- local(get(load("Data/results/exCPr/rEScp.Rdata")))[,2]
...
print(xtable(ESS, digits = c(0,rep(5,9)), align = rep("c",10)),
        sanitize.colnames.function = identity, 
        file = "latex/xtables/ESexSRBALL.tex",floating=FALSE)
print(xtable(USS, digits = c(0,rep(5,9)), align = rep("c",10)),
        sanitize.colnames.function = identity, 
        file = "latex/xtables/USexSRBALL.tex",floating=FALSE)
