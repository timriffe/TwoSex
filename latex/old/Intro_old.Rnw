...
<<ch1-sim1,echo=FALSE,results=hide>>=
sim1 <- function(){
	horizon <- 1000
	POP <- matrix(10,ncol=2,nrow=10)
	colnames(POP) <- c("males","females")
	Pi <- TFR <- vector(length=horizon)
	M1 <- c(rep(0,9),1)
	M1.5 <- c(rep(0,9),.5)
	M2 <- c(rep(0,9),.5,1)
	for (i in 1:horizon){
		M <- if (i<3) M1 else {if (i==3) M1.5 else M2}
		Deaths <- sum(POP*M)
		Births <- Deaths
		BMF <- rep(Births/2,2)
		POP <- POP-POP*M
		if (any(POP==0)){
			POP <- POP[-which(POP[,1]==0),]
		}
		POP <- rbind(BMF,POP)
		FR <- sum(POP[3:6,2])
		Rate <- Births/FR
		TFR[i] <- 4*sum((Rate*POP[3:6,2])/sum(POP[3:6,2]))
		Pi[i] <- sum(POP) # total pop, not returned
	}
	return(TFR)
}
sim1.results <- sim1()
...
<<label=fig1,fig=TRUE,echo=FALSE>>=
	...
	plot(1:500,
			...
			main="",
			xlab="Years",
			ylab="total pop. replacement TFR",
			cex.lab=1.5)
@
\end{center}
...
\begin{center}
% sim2
<<ch1-sim2, echo=FALSE, results=hide>>=
sim2 <- function(){
	set.seed(3586)
	o9 <- ((10/9)+10)
	horizon <- 1000
	POP <- matrix(c(9,11,9,11,9,11,9,11,9,11,rep(10,10)),ncol=2,nrow=10)
	colnames(POP) <- c("males","females")
	Pi <- TFR <- vector(length=horizon)
	M <- c(rep(0,9),1)
	Births <- 20
	for (i in 1:horizon){
		Deaths <- sum(POP*M)
		SRB <- ifelse(i%%2==0,(9/10),(o9/10))
		BMF <- c(Births*(SRB/(1+SRB)),Births*(1/(1+SRB)))
		POP <- POP-POP*M
		if (any(POP==0)){
			POP <- POP[-which(POP[,1]==0),]
		}
		POP <- rbind(BMF,POP)
		Ba <- unlist(table(sample(x=c(1:4),size=20,replace=T)))
		Bai <- rep(0,4)
		names(Bai) <- 1:4
		Bai[which(names(Bai)%in%names(Ba))] <- Ba[which(names(Ba)%in%names(Bai))]
		TFR[i] <- sum(Bai/POP[3:6,2])
		Pi[i] <- sum(POP)
	}
	return(TFR)
}
