...
\end{split}
\end{equation}
, giving the stable sex ratio at birth, $S$, as:
\begin{equation}
S = \frac{B^M}{B^F}
\end{equation}
...
\begin{equation}
b^\upsilon = \frac{1}{
            \splitfrac{\big(\frac{S}{1+S} \int _{y'=0}^\infty \int _{a'=y'}^\infty
            e^{-r^\upsilon a'} d_{a'}^F \dd a' \dd y'}{ + 
             \frac{1}{1+S} \int _{y=0}^\infty \int _{a=y}^\infty e^{-r^\upsilon a}
                      d_{a}^M \dd a \dd y\big)}  }                   
\end{equation}
, which can be used to derive the stable $e_x$-structure of males and females,
$c_y^\upsilon$ and $c_{y'}^\upsilon$, respectively:

\begin{equation}
c_{y'} = b^\upsilon \frac{1}{1+S} \int _{a'=y'}^\infty e^{-ra'} d_{a'}^F \dd
a'
\end{equation}
, and
\begin{equation}
c_{y} = b^\upsilon \frac{S}{1+S} \int _{a=y}^\infty e^{-ra} d_{a}^M \dd
a'
\end{equation}
, where:
\begin{equation}
...
\begin{equation}
S^T= \frac{\int c_{y} \dd y}{\int c_{y'} \dd y'}
\end{equation}

\paragraph{Construction of the corresponding projection matrix:} 
These formal relations establish coherence, but are less relevant in the
discrete world of applied demography, which best replicates the above with
projection matrices, modelled on that offered for the single-sex
$e_x$-structured case. The two-sex linear projection matrix combines the
projection of each sex jointly in a single instrument, the construction of which
is more involved than the single-sex case: Four times more involved to be
precise. Recall
