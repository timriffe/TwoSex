...
IPF-estimated intrinsic growth rates for the US and Spanish populations as compared with the single-sex
(sex-dominant) growth rates. Results are in general consistent and
believable. For the US population for these data it makes essentially no
difference whether one were to choose the arithmetic or harmonic means at the
initial marginal adjustment in the IPF prodedure\footnote{Both the arithmetic
...
Chapter~\ref{sec:ex2sexschoen}. In Chapter~\ref{sec:ex2sexschoen} mean functions
were used for exposures of each $y,y'$ combination, whereas in the IPF model the
mean is applied globally and then birth matrix counts are \textit{shaken up}
...
estimates are not so large as to be able to judge model preferability. It
is more informative to compare on the basis of other results, such as 
the intrinsic $e$SFR, which changes from the initial to stable states, both for the present model and
...
one may expect, differences between initial and stable rates are driven entirely 
...
the harmonic mean stable population.
